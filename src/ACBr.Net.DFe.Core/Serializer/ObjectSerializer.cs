...
namespace ACBr.Net.DFe.Core.Serializer
{
    internal class ObjectSerializer
    {
        ...
        private static IEnumerable<XObject> Serialize(PropertyInfo prop, object parentObject, SerializerOptions options)
        {
			...
			if (tag.Tipo == TipoCampo.Custom)
			{
				var serialize = parentObject.GetType().GetMethod($"Serialize{prop.Name}", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
				if (serialize != null && serialize.ReturnType == typeof(object))
					value = serialize.Invoke(parentObject, null);
			}

	        return new[] { PrimitiveSerializer.Serialize(value, tag, options) };
        }
